...
function ModalBorrowBook({ visible, onCancel, bookData }) {
    ...
    const maxReturnDate = today.add(1, 'month');
    // Effect to populate form with user and book data when the modal becomes visible.
    useEffect(() => {
        if (visible && dataUser) {
            // Đảm bảo Form đã mount xong vào DOM trong Modal thì mới đẩy dữ liệu
            setTimeout(() => {
                form.setFieldsValue({
                    quantity: 1,
                    ...
                    returnDate: dayjs().add(1, 'day'), // Trực tiếp dùng dayjs tránh lỗi dependency
                });
            }, 100);
        }
    }, [visible, dataUser, form]);
    ...
    const validateReturnDate = (_, value) => {
        ...
        if (value.isAfter(maxReturnDate, 'day')) {
            return Promise.reject(new Error('Thời gian mượn tối đa là 1 tháng!'));
        }
